...
if (isset($_GET['search'])) {
    ...
    // ดึงข ้อมูล eBooks โดยกรองตามค าค้นหา
    $sql = "SELECT * FROM ebooks WHERE name LIKE ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(["%$searchQuery%"]);
    $result = $stmt;
} else {
    // ดึงข ้อมูล eBooks ทั้งหมด
...
<head>
    ...
    <style>
        /* ก าหนดความกว้างของคอลัมน์ Description และ Actions */
        .description-column {
            width: 35%;
            /* ปรับตามที่ต้องการให ้แคบลง */
        }

        .actions-column {
            width: 15%;
            /* ปรับความกว้างให ้มากขึ้น */
        }
    </style>
</head>
...
by eBook name" value="<?= htmlspecialchars($searchQuery) ?>">
